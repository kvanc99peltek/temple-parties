...
| 2026-08-18 | Modal chrome (ModalWrapper) moved to v2: `temple-surface` + `border-white/10` hairline, 16px radius, plain black elevation — the purple glow/border violated rule 1. Backdrop is solid dim, no backdrop-blur (rule 3). All modals inherit |
| 2026-08-18 | Invite (post-GOING share) modal rebuilt from the kit: `✓ ON THE LIST` accent Pill replaces the 🎉 emoji, hero-scale uppercase title, FROM-THE-HOST body style, primary = the sticky bar's purple slot, quiet muted MAYBE LATER; left-aligned like every v2 surface |
