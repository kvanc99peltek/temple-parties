...
export default function PartyCard({
  ...
  const showAddress = revealedAddresses.includes(id);

  const handleRevealAddress = () => {
    setRevealedAddresses((prev) => [...prev, id]);
      ...
      <div className="p-5 sm:p-6">
        ...
        {!showAddress ? (
          <button
            ...
            className="text-white/50 text-sm sm:text-sm font-helvetica font-normal underline underline-offset-2 hover:text-white/70 transition-colors duration-200"
          >
            view address
          </button>
        ) : (
          <div className={`flex items-center gap-4 text-white/50 text-sm sm:text-sm font-helvetica font-normal ${justRevealed ? 'animate-reveal-right' : ''}`}>
