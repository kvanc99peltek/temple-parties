    ...
    <div className="flex items-center gap-2">
      <div className="flex gap-1.5">
        ...
        >
          <svg
            className={iconSize}
            viewBox="0 0 24 24"
            fill={userRating === 1 ? '#10B981' : 'none'}
            stroke={userRating === 1 ? '#10B981' : '#555'}
            strokeWidth={1.5}
          >
            <path
              strokeLinecap="round"
        ...
        >
          <svg
            className={iconSize}
            viewBox="0 0 24 24"
            fill={userRating === 0 ? '#EF4444' : 'none'}
            stroke={userRating === 0 ? '#EF4444' : '#555'}
            strokeWidth={1.5}
          >
            <path
              strokeLinecap="round"
      ...
      {size === 'sm' ? (
        <span className={`${textSize} text-white/40 font-helvetica`}>
          {ratingCount >= 5 ? `${Math.round(likePercentage)}% (${ratingCount})` : `(${ratingCount})`}
        </span>
      ) : (
        <span className={`${textSize} text-white/50 font-helvetica`}>
          {ratingCount >= 5 ? `${Math.round(likePercentage)}% liked this` : ratingCount > 0 ? `${ratingCount} ratings` : 'No ratings yet'}
        </span>
      )}
    </div>
  );
